...
def build_prompt(category_key: str, base_prompt: str) -> str:
    ...
    head = f"""[{category_key.upper()} PASS — OVER-INCLUDE MODE]

This pass focuses on {cat['cn']} ({category_key}) elements. Definition:

{cat['definition']}

**OVER-INCLUDE PHILOSOPHY**:
- Be EXHAUSTIVE. **Better to over-include than to miss.**
- If you see ANY visual element that COULD plausibly be {category_key}, return it. Even small/subtle ones. Even when borderline.
...
**Concrete examples of {category_key} elements**:
...
def call_route(category_key: str, base_prompt: str, img_b64: str):
        ...
        "messages": [
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"Page name: 奶茶盲盒-抽中页\nState: canonical\n\nThis is the [{category_key.upper()} PASS] running in OVER-INCLUDE mode. Return EVERY visual element that could plausibly be {category_key}. Borderline = INCLUDE. Cross-route overlap is OK. CRITICAL: bbox MUST be normalized 0-1 floats. Return JSON.",
                    },
    ...
    out_raw_path = os.path.join(OUT_DIR, f"poc2-over-{category_key}-raw.json")
    ...
    out_parsed_path = os.path.join(OUT_DIR, f"poc2-over-{category_key}.json")
...
def main():
    ...
    out_summary = os.path.join(OUT_DIR, "poc2-over-summary.json")
